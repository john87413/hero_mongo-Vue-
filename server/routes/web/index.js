    ...
    router.get('/heroes/list', async (req, res) => {
        ...
        const cats = await Category.aggregate([
            { $match: { parent: parent._id } },
            {
                $lookup: {
                    from: 'heroes',
                    localField: '_id', //內鍵 也就是在category裡面的
                    foreignField: 'categories', //外鍵 也就是在heroes裡面的
                    as: 'heroList'
                }
            }
        ])
